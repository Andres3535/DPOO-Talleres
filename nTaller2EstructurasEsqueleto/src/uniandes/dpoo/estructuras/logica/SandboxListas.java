...
public class SandboxListas
{
    ...
    public int contarEnterosRepetidos() {
        int contador = 0;
        List<Integer> listaTemp = new ArrayList<>(listaEnteros);
        for (int i = 0; i < listaTemp.size(); i++) {
            int elemento = listaTemp.get(i);
            listaTemp.remove(i);
            if (listaTemp.contains(elemento)) {
                contador++;
                listaTemp.removeAll(Collections.singleton(elemento));
            }
            listaTemp.add(i, elemento);
        }
        return contador;
    }
